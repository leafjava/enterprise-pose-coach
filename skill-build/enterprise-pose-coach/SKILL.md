...
|---|---|
| 模型层 | 成熟通用模型（RTMPose + ST-GCN）+ 训练出来的垂直模型（Best 权重）；训练起点是 `stgcn-mmfit-11cls-stride48@2026-05-29` |
...
| 实时纠正 | 规则引擎 + 阶段 + 错误码 | 第一次错误仅顶部红字，错误连续第 2 次触发语音（同一提示 5s 冷却） |
...
- 真实模型权重（`model/*.onnx`、`model/*.pth`）— 部署时挂载
- `src/` 推理与规则引擎源码
...
| 计数是否一致？ | §2：错误连续第 2 次才语音；幅度不足不计数；完整周期才 +1 |
